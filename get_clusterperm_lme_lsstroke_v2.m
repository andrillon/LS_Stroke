...
if size(model_est{2},2)==6 % ???
    for nDrug=1:3 % For each drug condition (being compared to placebo)
        real_clus=cell(1,2);
        for nsign=1:2 % For positive and negative relationships
            if nsign==1
                sig_elec=model_est{1}(model_est{1}(:,4)<clus_alpha & model_est{1}(:,3)>0 & model_est{1}(:,5)==nDrug,1); % Elec names for significant elecs with positive effects and group of interest
                tstat_elec=model_est{1}(model_est{1}(:,4)<clus_alpha & model_est{1}(:,3)>0 & model_est{1}(:,5)==nDrug,3); % t values as above
            else
                sig_elec=model_est{1}(model_est{1}(:,4)<clus_alpha & model_est{1}(:,3)<0 & model_est{1}(:,5)==nDrug,1); % As above but negative
                tstat_elec=model_est{1}(model_est{1}(:,4)<clus_alpha & model_est{1}(:,3)<0 & model_est{1}(:,5)==nDrug,3); % As above but negative
            end
        ...
        for nperm=1:totperm
            for nsign=1:2
                if nsign==1
                    sig_elec=model_est{2}(model_est{2}(:,4)<clus_alpha & model_est{2}(:,3)>0 & model_est{2}(:,5)==nperm & model_est{2}(:,6)==nDrug,1);
                    tstat_elec=model_est{2}(model_est{2}(:,4)<clus_alpha & model_est{2}(:,3)>0 & model_est{2}(:,5)==nperm & model_est{2}(:,6)==nDrug,3);
                else
                    sig_elec=model_est{2}(model_est{2}(:,4)<clus_alpha & model_est{2}(:,3)<0 & model_est{2}(:,5)==nperm & model_est{2}(:,6)==nDrug,1);
                    tstat_elec=model_est{2}(model_est{2}(:,4)<clus_alpha & model_est{2}(:,3)<0 & model_est{2}(:,5)==nperm & model_est{2}(:,6)==nDrug,3);
                end
            ...
            for nclus=1:size(real_clus{nsign},1)
                ...
                if nsign==1
                    if mean(this_tval<perm_tval{nsign})<montecarlo_alpha
                        nc=nc+1;
                        all_clus{nDrug}{nc}={'pos' real_clus{nsign}{nclus,1} this_tval sum(this_tval<perm_tval{nsign})/totperm};
                    end
                else
                    if mean(this_tval>perm_tval{nsign})<montecarlo_alpha
                        nc=nc+1;
                        all_clus{nDrug}{nc}={'neg' real_clus{nsign}{nclus,1} this_tval sum(this_tval>perm_tval{nsign})/totperm};
                    end
                end
            end
        end
