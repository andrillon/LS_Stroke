...
for idx = 1:length(IDs)
    ...
    for nF=1:length(file_names)
        ...
        if exist([path_save filesep 'allSW_trim_' file_names(nF).name(1:end-4) '_newfilt.mat'])==0 || redo==1
            [twa_results]=twalldetectnew_TA_newfilt(data,Fs,0);
            all_Waves=[];
            ...
            save([path_save filesep 'allSW_trim_' file_names(nF).name(1:end-4) '_newfilt'],'all_Waves','Fs','chan_labels');
        else
            load([path_save filesep 'allSW_trim_' file_names(nF).name(1:end-4) '_newfilt']);
        end
        ...
        save([path_save filesep 'SW_trim_' file_names(nF).name(1:end-4) '_newfilt'],'slow_Waves','Fs','chan_labels');
